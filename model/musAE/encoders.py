...
def build_encoder_sz():
	...
	for l in range(X_depth - 1):
		h_X = Bidirectional(
			LSTM(X_size, return_sequences=True, name=f"rec_X_{l}"), use_bias=True, use_cudnn=True,
			merge_mode="concat", name=f"bidirectional_X_{l}"
		)(h_X)
		#h_X = BatchNormalization(name=f"batchnorm_X_{l}")(h_X)

	h = Bidirectional(
			LSTM(X_size, return_sequences=False, name=f"rec_X_{X_depth - 1}"), use_bias=True, use_cudnn=True,
