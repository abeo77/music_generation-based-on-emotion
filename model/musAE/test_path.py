...
# from keras.layers.merge import Layer
...
class RandomWeightedAverage(Layer):
    def call(self, inputs, **kwargs):
        ...
        weights = K.random_uniform((batch_size, 1), minval=0.0, maxval=1.0)
...
class MusAE_GM():
    ...
    def train_v2(self, dataset):
        ...
        batches_path = os.path.join("/content/MusAE/dataset_16bar_intel/batches/X")
        ...
        bar = progressbar.ProgressBar(maxval=(self.n_epochs * self.len_dataset))
        pbc = 0
        pbc_tr = 0
        pbc_vl = 0
    ...
    aae_loss = self.adversarial_autoencoder.train_on_batch(
        [z_real, X.transpose(1, 0, 2, 3), label],
        [
            Y_drums, Y_bass, Y_guitar, Y_strings,
